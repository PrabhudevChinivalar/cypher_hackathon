...
export const generateOllamaResponse = async (message, courseData, conversationHistory) => {
  try {
...
// Analyze video content using Ollama
export const analyzeVideoContentOllama = async (videoUrl, courseData) => {
  try {
    const courseContext = createDetailedCourseContext(courseData);
...
5. Study tips for this specific video content`;
    ...
    console.error('Ollama video analysis error:', error);
    return "I can help you understand the video content. What specific aspects would you like to know more about?";
  }
};

// Generate study questions using Ollama
export const generateStudyQuestionsOllama = async (courseData) => {
  try {
    const courseContext = createDetailedCourseContext(courseData);
...
- Include both theoretical and practical questions`;
    ...
    console.error('Ollama study questions generation error:', error);
    return "I can help you create study questions for this course. What specific topics would you like to focus on?";
  }
};

// General knowledge assistant using Ollama
export const generalKnowledgeAssistant = async (message) => {
  try {
    const response = await ollama.chat({
      model: process.env.OLLAMA_MODEL || 'llama3.2',
      messages: [
...
- Always maintain a professional yet friendly tone`
    ...
    console.error('Ollama general knowledge error:', error);
...
// Fallback response generator (if Ollama fails)
